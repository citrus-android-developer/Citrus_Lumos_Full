...
> ★教訓★:測試翻紅時先問「紅的是實作還是測試」。這三條若當成實作缺陷去「修」,會把正確的行為改壞——正是 CLAUDE.md 那條「圖譜與行為事實衝突時不自動判圖譜為真」的同型情境,只是換成「測試與實作衝突時不自動判測試為真」。

## 未涵蓋

- **既有節點不回溯**。本次只讓新寫入寫得出四欄位;圖譜裡既有的決策仍是兩欄或更少,未批次回填。
